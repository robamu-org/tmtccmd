    ...
    if not os.path.isfile(json_cfg_path):
        ...
        with open(json_cfg_path, "r+") as file:
            try:
                ...
                _LOGGER.warning("JSON decode error, file format might be invalid. Replacing JSON")
                file.flush()
                file.truncate(0)
                file.seek(0)
                void_data = dict()
...
def save_to_json_with_prompt(
    ...
    save_to_json = input(f"Do you want to store the {name} to the configuration file? (y/n): ")
