...
class FrontendWorker(QRunnable):
    ...
    def spawn_for_opening_com_if(
        ...
        return cls(
            LocalArgs(WorkerOperationsCode.OPEN_COM_IF, (com_if_is_switched, com_if_key, hook)),
            shared_args,
        )

    @classmethod
    def spawn_for_cmd_path(cls, cmd_path: str, shared_args: SharedArgs) -> FrontendWorker:
        return cls(LocalArgs(WorkerOperationsCode.ONE_QUEUE_MODE, cmd_path), shared_args)
    ...
    def __setup(self, op_code: WorkerOperationsCode) -> bool:
        if op_code == WorkerOperationsCode.OPEN_COM_IF:
            ...
            if not set_success:
                LOGGER.warning(f"Could not set new communication interface {new_com_if}")
