...
class Service20FsfwTm(AbstractPusTm):
    ...
    def __common_checks(tm: PusTelemetry):
        ...
        if len(tm.source_data) < 4:
            raise ValueError("source data must include object ID (4 bytes) as minimal data")

    @classmethod
    def unpack(cls, raw_telemetry: bytes, timestamp_len: int) -> Service20FsfwTm:
        instance = cls.empty()
        instance.pus_tm = PusTelemetry.unpack(data=raw_telemetry, timestamp_len=timestamp_len)
