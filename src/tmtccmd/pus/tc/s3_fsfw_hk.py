...
@deprecated(
    version="v6.0.0rc0",
    reason="use diagnostic agnostic API if possible",
)
...
def create_disable_periodic_hk_command_with_diag(diag: bool, sid: bytes) -> PusTelecommand:
...
def __generate_periodic_hk_command_legacy(diag: bool, enable: bool, sid: bytes) -> PusTelecommand:
    app_data = bytearray(sid)
    if enable:
        if diag:
...
def modify_collection_interval(diag: bool, sid: bytes, interval_seconds: float) -> PusTelecommand:
    return create_modify_collection_interval_cmd_with_diag(diag, sid, interval_seconds)


def create_modify_collection_interval_cmd(sid: bytes, interval_seconds: float) -> PusTelecommand:
    app_data = bytearray(sid)
