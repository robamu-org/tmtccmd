...
class ParameterId:
    ...
    def pack(self) -> bytes:
        if self.domain_id > pow(2, 8) - 1:
            raise ValueError(f"domain ID {self.domain_id} too large, max value is {pow(2, 8) - 1}")
...
def __deserialize_unsigned_scalar_entry(
    ...
    if pfc == PfcUnsigned.FOUR_BYTES:
        ...
        raise NotImplementedError(f"Parsing of unsigned PTC {ptc} not implemented for PFC {pfc}")


def __deserialize_signed_scalar_entry(pfc: int, ptc: int, tm_data: bytes, param_len: int) -> int:
...
def create_matrix_float_parameter(
    ...
    for param_row in parameters:
        if len(param_row) != columns:
            raise ValueError("rows in passed mastrix do not have uniform number of columns")
